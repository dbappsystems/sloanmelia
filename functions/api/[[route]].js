...
// Handles all /api/* routes: auth, documents, achievements, colleges, scholarships, coach
// Bindings required: DB (D1), VAULT (R2)
// Secrets required: SM_EMAIL, SM_PASSWORD, JWT_SECRET, ANTHROPIC_API_KEY
...
async function handleVaultUpload(request, env) {
  ...
  await env.VAULT.put(key, file.stream(), {
...
- GPA: 3.84 weighted / 3.55 unweighted
...
- Achievements: Patient Enrichment Program (Founder), Mu Alpha Theta, Link Crew Mentor, Salvation Army Volunteer, Church Nursery Volunteer, Math Peer Tutor
- Target Schools: SIUE, University of Illinois Springfield, Missouri Baptist University, Saint Louis University

Help with college applications, essays, scholarship research, and forensic psychology study. Be warm, encouraging, and specific to Sloan's goals.${ vaultContext ? '\n\nContext from Sloan\'s vault:\n' + vaultContext : ''}`;
